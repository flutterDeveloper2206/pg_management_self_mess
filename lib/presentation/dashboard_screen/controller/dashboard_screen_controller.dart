...
class DashboardScreenController extends GetxController {
  ...
  Future<void> getConfig() async {
    await ApiService().callGetApi(
        ...
        showLoader: false,
        url: NetworkUrls.getConfigUrl).then((value) async {
        ...
        if (model.value.data != null && model.value.data!.isNotEmpty) {
          for (var item in model.value.data!) {
            if (item.configKey == 'total_day') {
              PrefUtils.setString(
                  StringConstants.totalDays, item.configValue ?? '');
            }
            if (item.configKey == 'eaten_day') {
              PrefUtils.setString(
                  StringConstants.eatenDays, item.configValue ?? '');
            }
            if (item.configKey == 'penalty') {
              PrefUtils.setString(
                  StringConstants.penalty, item.configValue ?? '');
            }
            if (item.configKey == 'feast_guest') {
              PrefUtils.setString(
                  StringConstants.feastGuest, item.configValue ?? '');
            }
            if (item.configKey == 'simple_guest') {
              PrefUtils.setString(
                  StringConstants.simpleGuest, item.configValue ?? '');
            }
          }
        }
      }
