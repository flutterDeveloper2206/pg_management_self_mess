  ...
  Future<void> calculateRate() async {
    await ApiService()
        .callGetApi(
          ...
          showLoader: true,
          url:
              '${NetworkUrls.calculateRateUrl}?month=${month.text}&year=${year.text}',
        )
        .then((value) async {
          if (value != null &&
            ...
            if (value != null &&
                value.body != null &&
                (value.body['state_code'] == 403 || value.statusCode == 403)) {
              WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
                AppFlushBars.appCommonFlushBar(
                  context: NavigationService.navigatorKey.currentContext!,
                  message:
                      value.body['message'] ??
                      "Bill is locked for the selected month. You cannot generate it again.",
                  success: false,
                );
              });
            }
  ...
  Future<void> generateBill(bool isLock) async {
    ...
    await ApiService()
        .callPostApi(
          ...
          headerWithToken: true,
          showLoader: true,
          url: NetworkUrls.generateBillUrl,
        )
        .then((value) async {
  ...
  Future<void> updateAllBill() async {
    ...
    await ApiService()
        .callPostApi(
          ...
          headerWithToken: true,
          showLoader: true,
          url: NetworkUrls.updateAllBillUrl,
        )
        .then((value) async {
